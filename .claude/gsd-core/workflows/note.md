...
- **Global scope**: `/home/user/test1/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/test1/.claude/notes/`)
...
2. Glob `/home/user/test1/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/test1/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/test1/.claude/notes/` — works in any directory
